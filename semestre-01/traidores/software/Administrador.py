...
class Administrador():
    ...
    def detectarTecla(self, event, estado):
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_i:
                ...
                self.traicionActiva = self.traicionActiva + 1
                if self.traicionActiva >= len(self.data['traiciones']):
                    self.traicionActiva = len(self.data['traiciones']) - 1
                return estado
        else:
            return estado
    ...
    def mostrarCampoValor(self, i, posY):
        self.ft_font.render_to(
            self.ventana,
            (400, posY),
            self.data['traiciones'][self.traicionActiva][i],
            constantes.MAGENTA)
    ...
    def mostrarAyuda(self):
        ...
        self.mostrarTitulo('administrador de traiciones')
        self.mostrarSubTitulo(constantes.ESTADO_AYUDA)
        self.mostrarTexto('a: ayuda', 300)
        self.mostrarTexto('d: ayuda', 400)
        self.mostrarTexto('i: inicio', 500)
        self.mostrarTexto('n: ayuda', 600)

    def mostrarDocumentar(self):
        self.ventana.blit(self.background, (0, 0))
        self.mostrarTitulo('administrador de traiciones')
        self.mostrarSubTitulo(constantes.ESTADO_DOCUMENTAR)

    def mostrarInicio(self):
        self.ventana.blit(self.background, (0, 0))
        self. mostrarTitulo('administrador de traiciones')
        self.mostrarSubTitulo(constantes.ESTADO_INICIO)
    ...
    def mostrarNavegar(self):
        self.ventana.blit(self.background, (0, 0))
        self.mostrarTitulo('administrador de traiciones')
        self.mostrarSubTitulo(constantes.ESTADO_NAVEGAR)
        iterador = 0
        for i in self.data['traiciones'][self.traicionActiva]:
            iterador = iterador + 1
    ...
    def correr(self):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
            elif self.estado == constantes.ESTADO_AYUDA:
                self.mostrarAyuda()
                self.estado = self.detectarTecla(event, self.estado)
            elif self.estado == constantes.ESTADO_DOCUMENTAR:
                self.mostrarDocumentar()
                self.estado = self.detectarTecla(event, self.estado)
            elif self.estado == constantes.ESTADO_INICIO:
                self.mostrarInicio()
                self.estado = self.detectarTecla(event, self.estado)
            elif self.estado == constantes.ESTADO_NAVEGAR:
                self.mostrarNavegar()
                self.estado = self.detectarTecla(event, self.estado)
            else:
                self.estado = self.detectarTecla(event, self.estado)
